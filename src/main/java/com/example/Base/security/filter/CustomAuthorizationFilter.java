...
public class CustomAuthorizationFilter extends OncePerRequestFilter { //OncePerRequestFilter가 application에 오는 모든 request를 다 인터셉트함(토큰확인 후 서비스 하기위함)
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        if(request.getServletPath().equals("/user/signin") || request.getServletPath().equals("/user/gosu/signin") || request.getServletPath().equals("/user/token/refresh") || request.getServletPath().equals("/test/subscribe") ) { //로그인이나 토큰 재발급은 필터를 거칠 필요없어서 바로 보내버린다.
